    ...
    <footer className="bg-brand-taupe text-brand-cream/80 py-20 border-t border-brand-shadow/20">
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-10">
                    ...
                    <div className="w-14 h-14 flex items-center justify-center rounded-full overflow-hidden bg-brand-cream border-2 border-brand-cream/50 p-1">
                        <img 
                            ...
                            onError={(e) => {
                                ...
                                e.currentTarget.parentElement!.innerHTML = '<span class="font-serif font-bold text-xl text-brand-taupe">BHK</span>';
                            }}
                        />
                    </div>
                    <span className="font-serif font-bold text-2xl tracking-tight text-brand-cream">BHKInterior.com</span>
                </div>
            ...
            <div className="flex flex-wrap justify-center gap-8 md:gap-12 text-brand-cream text-sm font-medium tracking-wide">
                {['Pricing', 'Gallery', 'About Us', 'Contact', 'Privacy Policy'].map(link => (
                    <a key={link} href="#" className="hover:text-brand-rose transition-colors duration-300 relative group">
                        {link}
